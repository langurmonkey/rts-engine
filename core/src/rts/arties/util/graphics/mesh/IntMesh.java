...
public class IntMesh implements Disposable {
	...
	final rts.arties.util.graphics.mesh.IntIndexData indices;
	boolean autoBind = true;
	final boolean isVertexArray;

	protected IntMesh(IntVertexData vertices, rts.arties.util.graphics.mesh.IntIndexData indices, boolean isVertexArray) {
	...
	public IntMesh(boolean isStatic, int maxVertices, int maxIndices, VertexAttribute... attributes) {
		...
		indices = new rts.arties.util.graphics.mesh.IntIndexBufferObject(isStatic, maxIndices);
		isVertexArray = false;
	...
	public IntMesh(boolean staticVertices, boolean staticIndices, int maxVertices, int maxIndices, VertexAttributes attributes) {
		...
		indices = new rts.arties.util.graphics.mesh.IntIndexBufferObject(staticIndices, maxIndices);
		isVertexArray = false;
	...
	private IntVertexData makeVertexBuffer (boolean isStatic, int maxVertices, VertexAttributes vertexAttributes) {
		if (Gdx.gl30 != null) {
			return new rts.arties.util.graphics.mesh.VertexBufferObjectWithVAO(isStatic, maxVertices, vertexAttributes);
		} else {
	...
	public IntMesh(VertexDataType type, boolean isStatic, int maxVertices, int maxIndices, VertexAttributes attributes) {
		...
		case VertexBufferObjectSubData:
			...
			indices = new rts.arties.util.graphics.mesh.IntIndexBufferObjectSubData(isStatic, maxIndices);
			isVertexArray = false;
			break;
		case VertexBufferObjectWithVAO:
			vertices = new rts.arties.util.graphics.mesh.VertexBufferObjectWithVAO(isStatic, maxVertices, attributes);
			indices = new rts.arties.util.graphics.mesh.IntIndexBufferObjectSubData(isStatic, maxIndices);
			isVertexArray = false;
			break;
		case VertexArray:
		default:
			vertices = new rts.arties.util.graphics.mesh.VertexArray(maxVertices, attributes);
	...
	/** Sets whether to bind the underlying {@link rts.arties.util.graphics.mesh.VertexArray} or {@link VertexBufferObject} automatically on a call to one of the
	...
	/** Binds the underlying {@link VertexBufferObject} and {@link rts.arties.util.graphics.mesh.IntIndexBufferObject} if indices where given. Use this with OpenGL
	...
	/** Unbinds the underlying {@link VertexBufferObject} and {@link rts.arties.util.graphics.mesh.IntIndexBufferObject} is indices were given. Use this with OpenGL
