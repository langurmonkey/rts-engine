...
import static arties.scene.ecs.Mapper.*;
...
public class UnitUpdateSystem extends IteratingSystem {
    ...
    private void updateStates(float deltaTime, Entity entity) {
        StateComponent stc = state.get(entity);
    ...
    private void updatePosition(float deltaTime, Entity entity) {
        ...
        MovementComponent mc = movement.get(entity);
        BodyComponent bc = body.get(entity);
        MapComponent mpc = map.get(entity);
        SteeringComponent sc = steering.get(entity);
        RenderableBaseComponent rbc = rbase.get(entity);
