...
public class QuadNode<T extends IBoundsObject> extends AStarNode<T> implements IMapCell<T> {
	...
	public List<Pair<QuadNode<T>, Float>> adjacentNodes = new ArrayList<Pair<QuadNode<T>, Float>>();
	...
	public void split() {
		if (isLeaf()) {
			...
			northWest = new QuadNode<T>(new Rectangle(bounds.getX(), bounds.getY(), hw, hh), this.owner, this);
			northEast = new QuadNode<T>(new Rectangle(bounds.getX() + hw, bounds.getY(), bounds.getWidth() - hw, hh),
					this.owner, this);
			southWest = new QuadNode<T>(new Rectangle(bounds.getX(), bounds.getY() + hh, hw, bounds.getHeight() - hh),
					this.owner, this);
			southEast = new QuadNode<T>(new Rectangle(bounds.getX() + hw, bounds.getY() + hh, bounds.getWidth() - hw,
	...
	public void genVisibilityGraphForNode(QuadNode<T> headNode, QuadNode<T> node) {
		...
		if (node.isLeaf()) {
			...
			List<QuadNode<T>> nodes = new ArrayList<QuadNode<T>>();
			...
			for (QuadNode<T> foundNode : nodes) {
				if (foundNode != node) {
					if (!foundNode.isBlocked()) {
						...
						node.adjacentNodes.add(new Pair<QuadNode<T>, Float>(foundNode, dist));
					} else {
						// Maximum cost for blocked nodes
						node.adjacentNodes.add(new Pair<QuadNode<T>, Float>(foundNode, Float.MAX_VALUE));
					}
				}
			}
		}
	...
	public Set<IMapCell<T>> findNodesWith(T o) {
		Set<IMapCell<T>> set = new HashSet<IMapCell<T>>();
