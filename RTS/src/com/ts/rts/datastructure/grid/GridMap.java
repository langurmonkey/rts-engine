...
public class GridMap<T extends IBoundsObject> implements IMap<T> {
	...
	public Set<IMapCell<T>> getCellsAround(float x, float y, int cellRadius) {
		...
		for (int i = (int) Math.max(0, col - cellRadius); i <= Math.min(rows - 1, col + cellRadius); i++) {
			for (int j = (int) Math.max(0, row - cellRadius); j <= Math.min(columns - 1, row + cellRadius); j++) {
				set.add(cells[i][j]);
			}
		}
		return set;
	...
	@Override
	public void add(T o) {
		Set<IMapCell<T>> set = getCellsAround(o.pos().x, o.pos().y, 2);
